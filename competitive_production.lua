...
competitive_production.on_player_created = function(event)
...
competitive_production.on_player_joined_game = function(event)
  local player = game.players[event.player_index]
  if not (player and player.valid) then return end
...
competitive_production.on_tick = function(event)
  check_end_of_round()
  check_chests()
  check_input_chests()
  check_clear_areas()
...
competitive_production.on_built_entity = function(event)
...
competitive_production.on_gui_click = function(event)
  local player = game.players[event.player_index]
  local gui = event.element
  if not (player and player.valid and gui and gui.valid and gui.name) then return end

  if gui.name == "team1_button" then
    player.force = game.forces.player
    table.insert(global.team1_players, player)
    table.insert(global.online_players, player)
    if not global.start_round_tick then
    ...
    update_team_buttons(player)
    game.print({"joined-team", player.name, "Blue"})
    return
  end

  if gui.name == "team2_button" then
    player.force = game.forces.player
    table.insert(global.team2_players, player)
    table.insert(global.online_players, player)
    if not global.start_round_tick then
      set_player(player, #global.online_players)
      give_starting_inventory(player)
    ...
    update_team_buttons(player)
    game.print({"joined-team", player.name, "Red"})
    return
  end

  if gui.name == "leaveteam_button" then
    player.force = game.forces.spectators
    for k, check_player in pairs (global.team1_players) do
      if check_player == player then
        table.remove(global.team1_players, k)
        break
      end
    end
    for k, check_player in pairs (global.team2_players) do
      if check_player == player then
        table.remove(global.team2_players, k)
        break
      end
    end
    update_team_buttons(player)
    game.print({"joined-spectators", player.name})
    return
  end
...
competitive_production.on_gui_closed = function(event)
  local gui = event.element
...
competitive_production.on_marked_for_deconstruction = function(event)
  local player = game.players[event.player_index]
